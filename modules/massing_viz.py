...
def _base_layout(title: str, lot_front: float, lot_depth: float,
    ...
    return dict(
        title=dict(text=title, font=dict(size=12, color="#1A1D2E"), x=0.5),
        scene=dict(
            xaxis=dict(title="", range=[-pad, lot_front + pad],
                       showbackground=False, gridcolor="#E8E3D4",
...
def floor_plate_fig(
    ...
    fig.update_layout(
        title=dict(text=label, font=dict(size=13, color="#1A1D2E"), x=0.5),
        xaxis=dict(
        ...
        paper_bgcolor="#FFFFFF",
        plot_bgcolor="#FFFFFF",
        legend=dict(x=1.01, y=0.99, font=dict(size=10, color="#3D4152")),
    )
    return fig
